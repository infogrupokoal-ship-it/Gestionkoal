...
def generate_receipt_pdf(
    ...
    story.append(
        Paragraph(f"Nombre: {client_details.get('name', 'N/A')}", styles["Normal"])
    )
    story.append(
        Paragraph(f"Teléfono: {client_details.get('phone', 'N/A')}", styles["Normal"])
    )
    story.append(
        Paragraph(f"Email: {client_details.get('email', 'N/A')}", styles["Normal"])
    )
    story.append(Spacer(1, 0.2 * inch))
    ...
    story.append(
        Paragraph(
            f"Descripción: {job_details.get('description', 'N/A')}", styles["Normal"]
        )
    )
    story.append(
        Paragraph(f"Estado: {job_details.get('status', 'N/A')}", styles["Normal"])
    )
    story.append(
        Paragraph(
            f"Método de Pago: {job_details.get('payment_method', 'N/A')}",
            styles["Normal"],
        )
    )
    story.append(
        Paragraph(
            f"Estado de Pago: {job_details.get('payment_status', 'N/A')}",
            styles["Normal"],
        )
    )
    story.append(Spacer(1, 0.2 * inch))
    ...
    if technician_details:
        ...
        story.append(
            Paragraph(
                f"Nombre: {technician_details.get('name', 'N/A')}", styles["Normal"]
            )
        )
        story.append(
            Paragraph(
                f"Teléfono: {technician_details.get('phone', 'N/A')}", styles["Normal"]
            )
        )
        story.append(Spacer(1, 0.2 * inch))
    ...
    # Amount Paid
    story.append(
        Paragraph(f"Monto Pagado: {job_details.get('amount', 0.0):.2f} €", styles["h2"])
    )
    ...
    story.append(
        Paragraph("Firma del Cliente / Confirmación de Pago", styles["Normal"])
    )
