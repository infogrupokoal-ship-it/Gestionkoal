...
def reassign_ticket(ticket_id):
    ...
    if ticket:
        client = db.session.execute(
            text("SELECT * FROM clientes WHERE id = :id"), {"id": ticket.cliente_id}
        ).fetchone()
    if not ticket or not client:
        return jsonify({"ok": False, "error": "Ticket o cliente no encontrado"}), 404

    assignee = suggest_assignee(db.session, ticket.tipo, ticket.prioridad, client)
    if assignee:
        db.session.execute(
            text("UPDATE tickets SET asignado_a = :uid WHERE id = :id"),
            {"uid": assignee["id"], "id": ticket_id},
        )
        db.session.commit()
        return jsonify({"ok": True, "assigned_to": assignee["username"]})
    return jsonify({"ok": False, "error": "No se encontró un asignado adecuado"})
...
def send_ticket_reply(ticket_id):
    ...
    if not phone:
        return jsonify({"ok": False, "error": "El cliente no tiene número de teléfono"}), 400
