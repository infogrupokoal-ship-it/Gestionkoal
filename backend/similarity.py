...
def is_duplicate_of_recent(
    db, client_id, title, desc, within_minutes=180, threshold=0.6
):
    ...
    rows = db.execute(
        text(
            "SELECT id, titulo, descripcion FROM tickets WHERE cliente_id = :cid AND fecha_creacion >= :since"
        ),
    ...
    for r in rows:
        ...
        descripcion = (
            getattr(r, "descripcion", None)
            if hasattr(r, "descripcion")
            else r["descripcion"]
        )
        score = max(
            simple_similarity(title, titulo or ""),
            simple_similarity(desc, descripcion or ""),
        )
        if score >= threshold:
