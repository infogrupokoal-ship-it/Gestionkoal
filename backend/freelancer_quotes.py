...
def add_freelancer_quote(job_id=None):
    ...
    tickets = db.execute(
        "SELECT id, descripcion FROM tickets ORDER BY descripcion"
    ).fetchall()
    providers = db.execute(
        "SELECT id, nombre FROM proveedores ORDER BY nombre"
    ).fetchall()
    freelancers_list = db.execute(
        "SELECT u.id, u.username FROM users u JOIN freelancers f ON u.id = f.user_id ORDER BY u.username"
    ).fetchall()
    ...
    if request.method == "POST":
        ...
        if error:  # Check for file upload errors
            ...
            if billing_entity_type == "Cliente":
                entity = db.execute(
                    "SELECT id FROM clientes WHERE id = ?", (billing_entity_id,)
                ).fetchone()
                if entity is None:
                    error = "ID de Cliente no válido."
            elif billing_entity_type == "Proveedor":
                entity = db.execute(
                    "SELECT id FROM providers WHERE id = ?", (billing_entity_id,)
                ).fetchone()
                if entity is None:
        ...
        if error is not None:
            ...
            try:
                ...
                db.commit()
                flash(
                    f"¡Presupuesto añadido correctamente con {len(uploaded_files_info)} archivos!"
                )
                return redirect(url_for("freelancer_quotes.list_freelancer_quotes"))
    ...
    if request.method == "POST":
        ...
        if error is not None:
            ...
            try:
                ...
                return redirect(
                    url_for(
                        "freelancer_quotes.edit_freelancer_quote", quote_id=quote_id
                    )
                )
            except Exception as e:
                flash(f"Ocurrió un error inesperado: {e}", "error")
...
def delete_file(file_id):
    ...
    file = db.execute(
        """SELECT f.id, p.id as quote_id FROM ficheros f
           ...
           WHERE f.id = ? AND p.freelancer_id = ?""",
        (file_id, g.user.id),
    ).fetchone()
    ...
    if file:
        try:
            ...
            if os.path.exists(file_path):
                try:
                    ...
                    current_app.logger.error(
                        f"Error al eliminar el archivo físico {file_path}: {e}"
                    )
            ...
            return redirect(
                url_for(
                    "freelancer_quotes.edit_freelancer_quote", quote_id=file["quote_id"]
                )
            )
        except Exception as e:
