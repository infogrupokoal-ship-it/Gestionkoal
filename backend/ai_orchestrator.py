...
def process_incoming_text(
    source: str, raw_phone: str, message_text: str = None, **kwargs
):
    # Backward-compat: accept text= as alias for message_text
    if message_text is None:
        message_text = kwargs.get("text")
    current_app.logger.debug(
        f"process_incoming_text started for phone: {raw_phone}, text: {message_text}"
    )
    ...
    try:
        ...
        client = db.session.execute(
            text("SELECT * FROM clientes WHERE telefono = :phone"), {"phone": phone}
        ).fetchone()
        if client is None:
            current_app.logger.debug(
                f"Client not found for phone {phone}. Attempting to create new client."
            )
            ...
            client_result = db.session.execute(
                text(
                    "INSERT INTO clientes (nombre, telefono, email, nif) VALUES (:nombre, :telefono, :email, :nif)"
                ),
                {"nombre": nombre, "telefono": phone, "email": email, "nif": nif},
            )
            client_id = client_result.lastrowid
            client = db.session.execute(
                text("SELECT * FROM clientes WHERE id = :client_id"),
                {"client_id": client_id},
            ).fetchone()
        ...
        current_app.logger.debug(
            f"Triage complete. Tipo: {tipo}, Prioridad: {prio}, Titulo: {titulo}"
        )

        # 3) Deduplicación
        current_app.logger.debug("Checking for deduplication...")
        dup_ticket_id = is_duplicate_of_recent(
            db.session, client.id, titulo, desc, within_minutes=180
        )
        if dup_ticket_id:
            current_app.logger.debug(
                f"Duplicate ticket found: {dup_ticket_id}. Logging and returning."
            )
            db.session.execute(
                text(
                    "INSERT INTO ai_logs (event_type,input,output,ticket_id,client_id,score) VALUES (:event_type, :input, :output, :ticket_id, :client_id, :score)"
                ),
                {
                    "event_type": "dedup",
                    "input": message_text,
                    "output": f"duplicate_of:{dup_ticket_id}",
        ...
        ticket_result = db.session.execute(
            text(
                "INSERT INTO tickets (cliente_id, source, tipo, prioridad, estado, titulo, descripcion, sla_due, creado_por) VALUES (:cliente_id, :source, :tipo, :prioridad, :estado, :titulo, :descripcion, :sla_due, :creado_por)"
            ),
            {
        ...
        policy_decision = evaluate_policies(client, {"tipo": tipo, "prioridad": prio})
        assignee = suggest_assignee(db.session, tipo, prio, client)
        if policy_decision.get("auto_assign") and assignee:
            current_app.logger.debug(
                f"Auto-assigning ticket {ticket_id} to {assignee['username']}"
            )
            db.session.execute(
                text(
                    "UPDATE tickets SET asignado_a = :asignado_a, estado = :estado WHERE id = :id"
                ),
        ...
        notification_message = (
            f"Se ha creado un nuevo ticket para ti: #{ticket_id} - {titulo}"
        )

        # Escoge un user_id válido de la tabla 'users'
        notify_user_id = assignee["id"] if assignee and assignee.get("id") else 1
        ...
        db.session.execute(
            text(
                "INSERT INTO ai_logs (event_type,input,output,ticket_id,client_id) VALUES (:event_type, :input, :output, :ticket_id, :client_id)"
            ),
            {
                "event_type": "triage",
                "input": message_text,
        ...
        msg = policy_decision.get(
            "reply_template", "Hemos recibido tu aviso. Te contactaremos pronto."
        )
        wa_response = WhatsAppClient().send_text(phone, msg)
        current_app.logger.debug(
            f"WhatsApp response sent. Status: {wa_response.get('status', 'unknown')}"
        )
        ...
        db.session.execute(
            text(
                "INSERT INTO whatsapp_message_logs (direction, to_number, message_body, status, error_info, whatsapp_message_id) VALUES (:direction, :to_number, :message_body, :status, :error_info, :whatsapp_message_id)"
            ),
            {
                "direction": "outbound",
                "to_number": phone,
