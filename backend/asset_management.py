...
def list_assets():
    db = get_db()
    if db is None:
        flash("Error de conexión con la base de datos.", "error")  # Redirect to a safe page, e.g., index or login

    assets = db.execute(
        "SELECT id, name, asset_type, status, description FROM assets ORDER BY name"
    ).fetchall()
...
def list_loans():
    db = get_db()
    loans = db.execute(
        """SELECT p.id, h.name as asset_name, u.username as user_name, p.loan_date, p.expected_return_date, p.status
           FROM asset_loans p JOIN assets h ON p.asset_id = h.id JOIN users u ON p.user_id = u.id ORDER BY p.loan_date DESC"""
    ).fetchall()
...
def add_loan():
    ...
    assets = db.execute(
        "SELECT id, nombre FROM herramientas ORDER BY nombre"
    ).fetchall()
    users = db.execute("SELECT id, username FROM users ORDER BY username").fetchall()

    if request.method == "POST":
        herramienta_id = request.form.get(
            "activo_id", type=int
        )  # Form still uses 'activo_id'
    ...
    return render_template(
        "asset_management/loan_form.html", loan=None, assets=assets, users=users
    )
...
def edit_loan(loan_id):
    db = get_db()
    loan = db.execute(
        "SELECT * FROM prestamos_herramienta WHERE id = ?", (loan_id,)
    ).fetchone()
    ...
    if request.method == "POST":
        ...
        devolucion = request.form.get("fecha_fin_prevista")
        estado_salida = request.form.get("estado")
        estado_entrada = request.form.get(
            "estado_entrada"
        )  # Assuming form might have this
        observaciones = request.form.get("notas")
        error = None
    ...
    return render_template(
        "asset_management/loan_form.html", loan=loan, assets=assets, users=users
    )
