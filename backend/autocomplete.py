...
# Autocompletion endpoint for providers
@bp.route("/providers")
def autocomplete_providers():
    """Provides autocompletion suggestions for provider names based on a query string."""
    query = request.args.get("q", "").strip()
    db = get_db()
    if db is None:
        return jsonify({"error": "Database connection error"}), 500
    providers = db.execute(
        "SELECT id, nombre FROM providers WHERE nombre LIKE ? ORDER BY nombre LIMIT 10",
        (f"%{query}%",),
    ).fetchall()
