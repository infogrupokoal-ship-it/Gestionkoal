...
def list_logs():
    ...
    try:
        rows = db.session.execute(
            text(
                "SELECT time as ts, message as details FROM error_log ORDER BY id DESC LIMIT 100"
            )
        ).fetchall()
        for r in rows:
            items.append(
        ...
        current_app.logger.info("No se encontró la tabla error_log o no se pudo leer")
    ...
    try:
        rows = db.session.execute(
            text(
                "SELECT created_at as ts, direction||':'||status as summary, message as details FROM whatsapp_logs ORDER BY id DESC LIMIT 100"
            )
        ).fetchall()
        for r in rows:
            items.append(
        ...
        current_app.logger.info(
            "No se encontró la tabla whatsapp_logs o no se pudo leer"
        )
    ...
    if not read_wa:
        try:
            rows = db.session.execute(
                text(
                    "SELECT timestamp as ts, direction||':'||status as summary, message_body as details FROM whatsapp_message_logs ORDER BY id DESC LIMIT 100"
                )
            ).fetchall()
            for r in rows:
                items.append(
                    {
                        ...
                        "summary": getattr(r, "summary", None),
                        "details": getattr(r, "details", None)
                        or getattr(r, "message_body", None),
                    }
                )
        except Exception:
            current_app.logger.info(
                "No se encontró la tabla whatsapp_message_logs o no se pudo leer"
            )

    try:
        rows = db.session.execute(
            text(
                "SELECT created_at as ts, event_type as summary, substr(output, 1, 200) as details FROM ai_logs ORDER BY id DESC LIMIT 100"
            )
        ).fetchall()
        for r in rows:
            items.append(
                {
                    ...
                    "summary": getattr(r, "summary", None),
                    "details": getattr(r, "details", None)
                    or getattr(r, "output", None),
                }
            )
    except Exception:
        current_app.logger.info("No se encontró la tabla ai_logs o no se pudo leer")

    try:
        rows = db.session.execute(
            text(
                "SELECT created_at as ts, 'notification' as summary, message as details FROM notifications ORDER BY id DESC LIMIT 100"
            )
        ).fetchall()
        for r in rows:
            items.append(
        ...
        current_app.logger.info(
            "No se encontró la tabla notifications o no se pudo leer"
        )
    ...
    if q:
        ...
        items = [
            it
            for it in items
            if qq in str(it.get("details") or "").lower()
            or qq in str(it.get("summary") or "").lower()
        ]
...
def toggles():
    ...
    return render_template(
        "audit/toggles.html",
        provider=cfg.get("WHATSAPP_PROVIDER")
        or os.environ.get("WHATSAPP_PROVIDER", "meta"),
        dry_run=(
            cfg.get("WHATSAPP_DRY_RUN")
            if cfg.get("WHATSAPP_DRY_RUN") is not None
