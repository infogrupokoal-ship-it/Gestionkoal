...
def _get_ai_response(user_message, chat_history):
    ...
    try:
        ...
        current_app.logger.warning(
            "Primario %s falló: %s. Probando fallback models/gemini-flash-latest",
            model_name,
            e,
        )
        try:
...
def content():
    ...
    return current_app.jinja_env.get_or_select_template(
        ["ai_chat/chat.html", "ai_chat.html"]
    ).render(
        ...
        if job_id:
            ...
            if db is None:
                current_app.logger.error(
                    "Error de conexión con la base de datos en el envío de mensajes del chat de IA."
                )
