...
def accounting_report():
    ...
    if not (
        g.user.has_permission("view_reports")
        or g.user.has_permission("manage_all_jobs")
    ):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            current_app.logger.error(
                f"Error generating accounting report: {e}", exc_info=True
            )
        ...
        if "download_csv" in request.form:
            ...
            for row in report_data:
                total_with_vat = row["amount"] + (
                    row["vat_amount"] if row["vat_amount"] is not None else 0
                )
                cw.writerow(
                    [
                        row["id"],
            ...
            output.headers["Content-Disposition"] = (
                "attachment; filename=informe_contable.csv"
            )
            output.headers["Content-type"] = "text/csv"
            return output

    return render_template(
        "accounting/report_form.html",
