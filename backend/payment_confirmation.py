...
def confirm_payment(ticket_id, token):
    ...
    if db is None:
        ...
        return render_template(
            "payment_confirmation/confirm.html",
    ...
    if ticket is None:
        ...
        return render_template(
            "payment_confirmation/confirm.html", message="Enlace no válido."
        )

    if ticket["estado_pago"] == "Pagado":
        flash("Este pago ya ha sido confirmado.", "info")
        return render_template(
            "payment_confirmation/confirm.html", message="Pago ya confirmado."
        )

    if ticket["payment_confirmation_token"] != token:
        flash("Token de confirmación no válido.", "error")
        return render_template(
            "payment_confirmation/confirm.html", message="Token no válido."
        )

    if datetime.now() > datetime.strptime(
        ticket["payment_confirmation_expires"], "%Y-%m-%d %H:%M:%S"
    ):
        flash("El enlace de confirmación ha expirado.", "error")
        return render_template(
            "payment_confirmation/confirm.html", message="Enlace expirado."
        )

    if request.method == "POST":
        try:
            db.execute(
                "UPDATE tickets SET estado_pago = ? WHERE id = ?", ("Pagado", ticket_id)
            )
            db.commit()
            flash("¡Pago confirmado con éxito! Gracias.", "success")
            return render_template(
                "payment_confirmation/success.html", ticket_id=ticket_id
            )
