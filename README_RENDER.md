...
2. Asegurate de estos campos:
   ...
   - **Start Command**: `gunicorn "backend:create_app()" --bind 0.0.0.0:$PORT --workers 2 --timeout 120`
...
5. Tras el primer deploy, abre shell (Render) y ejecuta:
   - `FLASK_APP=backend:create_app flask init-db`
ECHO est  desactivado.
## Opcion B: Postgres
...
4. Ejecuta `FLASK_APP=backend:create_app flask init-db` en la shell de Render.
ECHO est  desactivado.
## Notas
...
- `Procfile` debe ser: `web: gunicorn "backend:create_app()" --bind 0.0.0.0:$PORT --workers 2 --timeout 120`
