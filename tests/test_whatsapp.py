...
def test_webhook_verify(client, monkeypatch):
    ...
    rv = client.get(
        "/webhooks/whatsapp/?hub.mode=subscribe&hub.verify_token=abc&hub.challenge=123"
    )
...
def test_webhook_verify_fail(client, monkeypatch):
    monkeypatch.setenv("WHATSAPP_VERIFY_TOKEN", "abc")
    rv = client.get(
        "/webhooks/whatsapp/?hub.mode=subscribe&hub.verify_token=xyz&hub.challenge=123"
    )
...
def test_webhook_alias_dry_run_logs_payload(client, monkeypatch):
    ...
    with client.application.app_context():
        before = db.session.execute(
            text("SELECT COUNT(*) FROM whatsapp_message_logs")
        ).scalar()
...
def test_send_text_dry_run_no_auth(client, monkeypatch):
    ...
    rv = client.post(
        "/notifications/wa_test", json={"to": "34600111222", "text": "ping"}
    )
